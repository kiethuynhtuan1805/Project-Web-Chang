...
export default function SearchBar() {
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            ...
            if (element && !element.contains(event.target) && searchIcon && !searchIcon.contains(event.target)) {
                setActive(false);
            }
        };
    ...
    return (
        <div className={`SearchBar ${active ? 'active' : ''}`}>
            <div className='search-bar' id='search-bar'>
                <input type="text" className='search-input' value={searchValue} onChange={handleInputChange} />
